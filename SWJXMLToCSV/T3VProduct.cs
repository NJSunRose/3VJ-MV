...
namespace SWJXMLToCSV
{
    public class T3VProduct
    {
        ...
        /// <returns></returns>
        public static List<Cabinet> LoadFromXML(string base64String)
        {
            var contents = Convert.FromBase64String(base64String);

            XmlDocument doc = new XmlDocument();

            doc.Load(new MemoryStream(contents));
    ...
    public class Panel
    {
        ...
        internal void LoadFromXmlNode(XmlNode xnpanel, Cabinet parentcabinet)
        {
            try
            {
                ...
                MulPanels = false;
                //if (xnpanel.Attributes["CabinetNo"] != null)
                //{
                        ...
                        if (xnpanel.ChildNodes[xmlsubnum].Name == "Panels")
                        {
                            foreach (XmlNode xmlsub in xnpanel.ChildNodes[xmlsubnum].ChildNodes)
                            {
                                if (xmlsub.Name == "Panel")
                                {
                                    ...
                                    childpanel.LoadFromXmlChildNode(xmlsub, parentcabinet);
        ...
        private void LoadFromXmlChildNode(XmlNode xmlchildnode,Cabinet parentcabinet)
        {
            try
            {
        ...
        private void LoadFromXmlsubsubChildNode(XmlNode xmlsubsubsub, Cabinet parentcabinet)
        {
            try
            {
                cabinet = parentcabinet;
